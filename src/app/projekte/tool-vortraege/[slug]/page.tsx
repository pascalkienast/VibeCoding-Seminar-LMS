...
export default function ToolDetailPage({ params }: Params) {
  ...
  const loadTool = async () => {
    ...
    const { data: commentsData } = await supabase
      .from("presentation_tool_comments")
      .select("*")
      .eq("tool_id", toolData.id)
  ...
  const handleJoin = async () => {
    ...
    const { error } = await supabase.from("presentation_tool_presenters").insert({
      tool_id: tool.id,
  ...
  const handleAddComment = async () => {
    ...
    const supabase = getSupabaseBrowserClient();
    const { error } = await supabase.from("presentation_tool_comments").insert({
      tool_id: tool.id,
      ...
      <div className="max-w-4xl mx-auto space-y-6">
          ...
          {presenters.length > 0 ? (
            <div className="space-y-2">
              {presenters.map((p, index) => (
                <div
                  key={p.id}
                  className="px-4 py-3 rounded-lg text-sm bg-purple-50 dark:bg-purple-950 border border-purple-200 dark:border-purple-800"
                >
                  <div className="flex items-center justify-between">
        ...
        <div className="card bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800">
          <p className="text-sm text-amber-900 dark:text-amber-100">
            💡 <strong>Hinweis:</strong> Diese Seite kann nur von Admins bearbeitet werden. 
