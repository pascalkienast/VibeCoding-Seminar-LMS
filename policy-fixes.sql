...
create policy forum_categories_admin_write on public.forum_categories for all
  using (public.is_admin(auth.uid()))
  with check (public.is_admin(auth.uid()));

drop policy if exists forum_topics_admin_write on public.forum_topics;
create policy forum_topics_admin_write on public.forum_topics for all
  using (public.is_admin(auth.uid()))
  with check (public.is_admin(auth.uid()));

drop policy if exists forum_posts_admin_write on public.forum_posts;
create policy forum_posts_admin_write on public.forum_posts for all
  using (public.is_admin(auth.uid()))
  with check (public.is_admin(auth.uid()));
